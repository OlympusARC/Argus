...
def _row(p: Posting, ts: int, source: str) -> tuple:
    ...
    return (
        ...
        # Truncated to the day, as in feed/diff -- at per-second resolution the
        # fallback encodes poll order, and sorting by posted_at then returns one
        # company's whole board before the next one starts.
